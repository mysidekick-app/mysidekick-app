...
const row1Modules: FinanceModule[] = [
  {
    key: 'income',
    label: 'Money in',
    route: '/modules/finances/income',
    icon: ArrowDownToLine,
  },
  {
    key: 'expense',
    label: 'Money out',
    route: '/modules/finances/expense',
...
const row2Modules: FinanceModule[] = [
  {
    key: 'savings',
    label: 'Savings',
    route: '/modules/finances/savings',
    icon: PiggyBank,
  },
  {
    key: 'debt',
    label: 'Debts',
    route: '/modules/finances/debt',
    icon: CreditCard,
  },
  {
    key: 'split',
    label: 'Split',
    route: '/modules/finances/split',
    icon: UsersRound,
  },
];
...
export default function FinancesDashboard() {
  ...
  const load = useCallback(async () => {
    ...
    const [
      { data: transactionRows, error: transactionErr },
      { data: savingsRows, error: savingsErr },
    ] = await Promise.all([
      supabase
        .from('finance_transactions')
        .select(
          'id, kind, title, amount, category, transaction_date'
        )
    ...
    if (transactionErr || savingsErr) {
      setError('Your financial data could not be loaded.');
    } else {
      setTransactions(
        (transactionRows ?? []) as FinanceTransaction[]
      );

      setSavings(
        (savingsRows ?? []) as SavingsGoal[]
      );
    }
          ...
          <View style={styles.chart}>
            {chartValues.map((item) => {
              ...
              return (
                ...
                >
                  {/* 
                    The bar and amount are positioned
                    together at the bottom.
                    This means each amount sits directly
                    above its own bar instead of all
                    amounts being aligned at the top.
                  */}
                  <View
                    style={[
                      styles.barColumn,
...
const styles = StyleSheet.create({
  ...
  /*
   * Fixed-height area containing each individual bar.
   * The amount is positioned relative to its own bar,
   * so amounts move with their corresponding bar.
   */
  barColumn: {
    width: '64%',
    minWidth: 22,
  ...
  /*
   * Transparent track:
   * there is no white/grey filler behind the bar.
   * Only the actual progress amount is visible.
   */
  bar: {
    width: '100%',
    borderRadius: 8,
